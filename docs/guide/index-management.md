...
1. Confirm the destination does not exist and validate the concrete source. Closed indexes, data-stream backing indexes, system indexes, ILM-managed indexes, CCR followers, non-standard index modes, and indexes with `_source` disabled are rejected. Open a closed index explicitly before upgrading it. A pre-existing write-only block is preserved; read, metadata, and read-only blocks must be removed by the operator first.
