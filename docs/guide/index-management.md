...
2. The response is keyed by concrete backing index names and includes `index.version.created`, so alias resolution, physical-name discovery, compatibility detection, ownership authentication, and selection of all partitions happen in that single request. `index.version.created` is not mapping metadata, so folding this into the normal mapping lookup would add payload and startup I/O rather than remove it. A discovered physical index is reported only when no other registered configuration index claims it more strongly; this keeps overlapping names such as `events`, `events-error`, and `reindexed-v8-events` from claiming each other's physical indexes during preflight, upgrade, or wildcard deletes.
...
1. Validate reindex batch/throttle options, confirm the destination does not exist, and validate the concrete source before adding any block. Source ownership and schema precedence are revalidated at execution time, after progress callbacks and immediately before the write block. Closed indexes, data-stream backing indexes, system indexes, ILM-managed indexes, CCR followers, non-standard index modes, and indexes with `_source` disabled are rejected. Open a closed index explicitly before upgrading it. For versioned indexes, an older schema version still behind the logical alias is a pending schema migration and must be schema-reindexed first; an unaliased retained old version is upgraded independently. A pre-existing write-only block is preserved; read, metadata, and read-only blocks must be removed by the operator first.
...
3. Create `reindexed-v{serverMajor}-{canonicalSourceName}` with the `_create_from` API. This requires Elasticsearch 8.18 or later and copies the source settings and mappings without reconstructing them from application configuration. Because destination-name templates are applied during index creation, fail before copying documents if they introduce aliases, mappings, or explicit settings not present on the source. A failed create is never followed by automatic cleanup: a lost response or an error response cannot prove the destination was left untouched, so the outcome is treated as unknown and the source write block stays in place.
...
5. Add a private hidden ownership alias for conservative crash recovery. Require a clean task result, explicit target refresh, zero failed count/refresh shards, and exact source/task/target document counts. If the start request can no longer prove a task ID, retain both the destination and source write block until the matching Elasticsearch tasks are inspected; an untracked task is never followed by automatic cleanup. Restore replicas, refresh interval, and both pipeline settings exactly—including restoring originally absent settings as unset—keep the destination write blocked, and wait for its primary shards to become available.
...
7. Reconfirm that the destination still has only the ownership alias, then atomically remove that marker, add every source alias plus one canonical old physical-name alias to the destination, and remove the concrete source. Generated compatibility names from earlier majors are replaced, not accumulated as aliases.
...
The compatibility upgrader is a dedicated component. The shared schema reindexer rejects filtered/routed source aliases because it cannot safely reproduce their subset semantics during catch-up. For ordinary aliases it fails closed when alias metadata cannot be read, re-reads the complete alias set at cutover, performs the atomic alias move after the first pass, removes the canonical compatibility alias for the retired schema version, then catches up from the resolved concrete old index so the moved alias cannot redirect the second pass into the destination. An initially empty source gets a full post-cutover pass, closing the race where its first document arrives after the initial sample but before alias movement.

If an interrupted attempt leaves a destination behind, the next run stops. Use `InspectIndexCompatibilityUpgradeAsync()` to read the physical topology, aliases, write blocks, ownership marker, and matching active reindex tasks. Supply the original pre-upgrade concrete source from preflight, not the generated current-major destination. The source must structurally belong to the supplied configured index; cross-index names and index expressions are rejected before any request or lock, and a source equal to the deterministic destination is rejected before topology classification. A completed topology is recognized only when the destination has the canonical source alias and the pre-cutover ownership marker is absent. `RecoverIndexCompatibilityUpgradeAsync()` acquires the same distributed lock and deletes a pre-cutover destination only when it carries the exact hidden ownership marker and task termination is known. A partial Elasticsearch task listing (`node_failures` or `task_failures`) leaves task termination unknown, so recovery fails closed without deleting the destination. An unmarked or otherwise aliased destination is ambiguous and is never deleted automatically. Removing a surviving write block requires an explicit `removeWriteBlock: true` because the library cannot infer whether an administrator or the interrupted attempt originally owned that block.
...
- **`InspectIndexCompatibilityUpgradeAsync(index, sourceIndex, cancellationToken)`** is read-only. Keep the original canonical physical source name from preflight; inspection derives the current-major destination, reports whether target ownership is confirmed, and classifies the topology as `Ready`, `SourceWriteBlocked`, `Interrupted`, `InProgress`, `Completed`, `CompletedWriteBlocked`, `Missing`, or `Ambiguous`.
...
Cancellation or a pre-cutover failure cancels the Elasticsearch task, removes the known destination, and removes only the write block added by this operation. Cleanup independently confirms that the destination is absent before it removes the source write block; a failed or ambiguous delete therefore leaves the source blocked. If cleanup cannot be confirmed, the exception explicitly instructs the operator to inspect both indexes and the source write block. Once the atomic alias/delete action is dispatched, a lost response is treated as an uncertain cutover: no index is deleted automatically, and the aliases and both physical indexes must be inspected before retrying. Cancellation cannot roll a completed cutover back.

The same fail-closed rule applies to destination creation. If the `_create_from` request fails for any reason — a lost response, or even an error response that cannot prove the destination was left untouched — the outcome is treated as unknown: the destination (if any) is retained, the source write block stays in place, and no automatic cleanup runs. Retry attempts stop at the destination-exists check; use `InspectIndexCompatibilityUpgradeAsync()` before removing anything.
...
Ownership of physical indexes follows configured names, not string shapes. Discovery wildcards include compatibility-wrapped forms, so configurations whose names overlap after generated-prefix or error-suffix stripping (for example `events`, `events-error`, and `reindexed-v8-events`) could otherwise claim each other's physical indexes. A candidate is owned only when no other registered configuration index claims it more strongly: exact/native family membership outranks generated-name matching, and among native families the longest configured name wins. Ambiguous candidates are excluded from preflight, rejected as upgrade/recovery sources, and skipped by wildcard deletes — including `DeleteAsync()` — so one configuration can never reindex or delete another configuration's data. Library-generated `{name}-error` partitions carry the exact hidden `.foundatio-reindex-error` alias. The suffix alone never proves ownership: a legacy or foreign unmarked `-error` candidate fails preflight and is never upgraded or deleted automatically. After an operator verifies provenance, they can resolve that collision explicitly before retrying.
