...
1. Calling `GetIndexCompatibilityAsync()` reads the connected server version and issues one settings-only request. Plain indexes resolve their configured name plus the generated `{name}-error` partition. Versioned indexes scan every structurally valid `name-v*` physical index (including generated `-error` partitions and retained old schema versions). Daily and monthly indexes scan every structurally valid partition, including expired partitions that maintenance has not deleted, hidden indexes, and their `-error` twins. Retention is not assumed to have completed.
2. The response is keyed by concrete backing index names and includes `index.version.created`, so alias resolution, physical-name discovery, compatibility detection, and selection of all partitions happen in that single settings request. `index.version.created` is not mapping metadata, so folding this into the normal mapping lookup would add payload and startup I/O rather than remove it. A discovered physical index is reported only when no other registered configuration index claims it more strongly; this keeps overlapping names such as `events` and a legitimately configured `reindexed-v8-events` from claiming each other's physical indexes during preflight, upgrade, or wildcard deletes.
...
Ownership of physical indexes follows configured names, not string shapes. Discovery wildcards include compatibility-wrapped forms, so two configurations whose names overlap after generated-prefix stripping (for example `events` and a legitimately named `reindexed-v8-events`) could otherwise claim each other's physical indexes. A candidate is owned only when no other registered configuration index claims it more strongly: a family that contains the name natively (without stripping) outranks one that matches only after stripping, and among native families the longest configured name wins. Ambiguous candidates are excluded from preflight, rejected as upgrade/recovery sources, and skipped by wildcard deletes — including `DeleteAsync()` — so one configuration can never reindex or delete another configuration's data. Library-generated `{name}-error` partitions (written by the reindexer when documents fail to copy) are part of every family: they are discovered, reported, upgraded like any other source, and verified after cutover, so a failed schema reindex cannot leave an old-format partition that silently blocks the next Elasticsearch major.
