...
5. Require a clean task result, explicit target refresh, zero failed count/refresh shards, and exact source/task/target document counts. If the start request can no longer prove a task ID, retain both the destination and source write block until the matching Elasticsearch tasks are inspected; an untracked task is never followed by automatic cleanup. Restore replicas, refresh interval, and both pipeline settings, keep the destination write blocked, and wait for its primary shards to become available.
