...
- **Automatic Lock Renewal**: The handler renews its caller-owned work-item lease independently every 30 seconds with bounded attempts, and retains progress-boundary renewal. Loss cancels linked work; already-dispatched requests still require reconciliation
...
- **Durable completion records**: A finished migration writes a record to `foundatio-reindex-completions` before the queue item is acknowledged and before the source is deleted. For either ordering, a promoted destination without a matching record produces `ReindexCompletionUnknownException` rather than acknowledgment or replay into the live destination. The same rule applies to quiesced migrations.
...
::: warning Quiesced redelivery does not establish the earlier attempt's provenance
A promoted work item without a matching completion record is refused even when `QuiesceSource = true`. The flag describes the current request; it does not prove that the attempt which moved the alias blocked writes and verified its copy. Do not change the flag to clear an unknown completion result. Recovery requires evidence about the earlier attempt and inspection of both indexes, not an inference from the new work item's settings. No completion record is fabricated from the flag.
:::
