...
namespace Foundatio.Repositories.Elasticsearch.Jobs {
    public class CleanupSnapshotJob : IJob {
        ...
        private async Task DeleteOldSnapshotsAsync(string repo, TimeSpan maxAge, CancellationToken cancellationToken) {
            ...
            if (result.IsValid)
                _logger.LogInformation($"Retrieved list of {snapshots.Count} snapshots from {repo} in {sw.Elapsed.ToWords(true)}");
            else
            ...
            _logger.LogInformation($"Selected {snapshotsToDelete.Count} snapshots for deletion");
            ...
            foreach (var snapshot in snapshotsToDelete) {
                ...
                _logger.LogInformation($"Acquiring snapshot lock to delete {snapshot.Name} from {repo}");
                try {
                    await _lockProvider.TryUsingAsync("es-snapshot", async t => {
                        _logger.LogInformation($"Got snapshot lock to delete {snapshot.Name} from {repo}");
                        sw.Restart();
        ...
        public virtual Task OnSnapshotDeleted(string snapshotName, TimeSpan duration) {
            _logger.LogInformation($"Completed delete snapshot {snapshotName} in {duration.ToWords(true)}");
