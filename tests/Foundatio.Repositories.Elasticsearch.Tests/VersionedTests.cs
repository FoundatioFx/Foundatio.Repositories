...
namespace Foundatio.Repositories.Elasticsearch.Tests {
    public sealed class VersionedTests : ElasticRepositoryTestBase {
        ...
        public async Task CanUsePagingAsync() {
            ...
            var results = await _employeeRepository.GetAllAsync(new CommandOptions().WithLimit(PAGE_SIZE));
        ...
        public async Task CanUsePagingWithCachingAsync() {
            ...
            var results = await _employeeRepository.GetAllByCompanyAsync("1", new CommandOptions().WithLimit(PAGE_SIZE).EnableCache());
            Assert.True(results.HasMore);
            Assert.Equal(PAGE_SIZE, results.Documents.Count);
            ...
            results = await _employeeRepository.GetAllByCompanyAsync("1", new CommandOptions().WithLimit(PAGE_SIZE).EnableCache());
            Assert.True(results.HasMore);
            Assert.Equal(PAGE_SIZE, results.Documents.Count);
            Assert.Equal(1, _cache.Count);
            ...
            results = await _employeeRepository.GetAllByCompanyAsync("1", new CommandOptions().WithLimit(20).EnableCache());
            Assert.True(results.HasMore);
        ...
        public async Task CanUseSnapshotPagingAsync() {
            ...
            var results = await _employeeRepository.GetAllAsync(new CommandOptions().WithLimit(PAGE_SIZE).WithSnapshotPaging());
            Assert.True(results.HasMore);
        ...
        public async Task CanUseSnapshotWithScrollIdAsync() {
            ...
            do {
                ...
                results = await _employeeRepository.GetAllAsync(new CommandOptions().WithScrollId(results));
        ...
        public async Task UpdateAllWithPageLimitAsync() {
            ...
            await _client.RefreshAsync(Indices.All);
            var results = await _employeeRepository.GetAllByCompanyAsync("1", new CommandOptions().WithLimit(NUMBER_OF_EMPLOYEES));
