...
namespace Foundatio.Repositories.Elasticsearch.Tests {
    public sealed class RepositoryTests : ElasticRepositoryTestBase {
        ...
        public async Task CanQueryByDeleted() {
            ...
            var allEmployees = await _employeeRepository.FindAsync(q => q.SoftDeleteMode(SoftDeleteQueryMode.All));
            Assert.Equal(2, allEmployees.Total);

            var onlyDeleted = await _employeeRepository.FindAsync(q => q.SoftDeleteMode(SoftDeleteQueryMode.DeletedOnly));
            Assert.Equal(1, onlyDeleted.Total);
            Assert.Equal(employee1.Id, onlyDeleted.Documents.First().Id);

            var nonDeletedEmployees = await _employeeRepository.FindAsync(q => q.SoftDeleteMode(SoftDeleteQueryMode.ActiveOnly));
        ...
        public async Task CanQueryByDeletedSearch() {
            ...
            var onlyDeleted = await _employeeRepository.FindAsync(q => q.SoftDeleteMode(SoftDeleteQueryMode.All).FilterExpression("isDeleted:true"));
            Assert.Equal(1, onlyDeleted.Total);
            Assert.Equal(employee1.Id, onlyDeleted.Documents.First().Id);
        ...
        public async Task CanGetAggregationsAsync() {
            ...
            var result = await _dailyRepository.CountBySearchAsync(q => q.AggregationsExpression("cardinality:companyId max:createdUtc"));
        ...
        public async Task CanGetDateAggregationAsync() {
            ...
            var result = await _dailyRepository.CountBySearchAsync(q => q.AggregationsExpression("date:(createdUtc min:createdUtc)"));
            ...
            result = await _dailyRepository.CountBySearchAsync(q => q.AggregationsExpression("date:(createdUtc~1h^-3h min:createdUtc)"));
            Assert.Equal(1, result.Aggregations.Count);
        ...
        public async Task CanGetGeoGridAggregationAsync() {
            ...
            var result = await _employeeRepository.CountBySearchAsync(q => q.AggregationsExpression("geogrid:(location~6 max:age)"));
            Assert.Equal(1, result.Aggregations.Count);
