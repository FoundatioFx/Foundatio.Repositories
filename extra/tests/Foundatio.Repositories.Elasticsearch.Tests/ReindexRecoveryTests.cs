...
public sealed class ReindexRecoveryTests(ITestOutputHelper output) : ElasticRepositoryTestBase(output)
{
    ...
    private async Task PutDocumentAsync(string index, string id, bool deleted = false, string? routing = null)
        => await SendAsync(HttpMethod.PUT, $"/{index}/_doc/{id}?refresh=true" + (routing is null ? "" : "&routing=" + routing),
