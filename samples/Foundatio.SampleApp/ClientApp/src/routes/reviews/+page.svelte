<script>
    ...
    async function getReviews() {
        ...
        if (categories.length > 0 && tags.length > 0)
            filter = "(" + categories.map(c => "category:\"" + c + "\"").join(' AND ') + ") AND (" + tags.map(t => "tags:\"" + t + "\"").join(' AND ') + ")";
        else if (categories.length > 0)
            filter = categories.map(c => "category:\"" + c + "\"").join(' AND ') ;
        else if (tags.length > 0)
            filter = tags.map(t => "tags:\"" + t + "\"").join(' AND ');
...
{#each data.categoryCounts as category}
    <button type="button" class="btn btn-outline-secondary btn-sm" on:click={() => setCategory(category.name)}>
