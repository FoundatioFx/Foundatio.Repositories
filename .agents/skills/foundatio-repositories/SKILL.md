...
- **v8+ Live `SearchAfterPaging()` requires a sort**: The repository adds the model id's sort-safe mapped field (such as `id.sort`) when possible. If the model has no `IIdentity` or the index sets `HasSortableIdField = false`, Live mode requires an explicit stable, unique sort and throws `QueryValidationException` when none exists. Point-in-time mode always makes Elasticsearch's implicit `_shard_doc` tiebreaker explicit after caller sorts, keeping the complete sort tuple reversible for backward cursors.
- **Search-after direction is exclusive and disabling resets the session**: Setting an after cursor clears any before cursor, and vice versa. `SearchAfterPaging(false)` clears cursors, mode, and stored point-in-time state; close an abandoned point-in-time result before disabling or let its keep-alive expire.
- **Externally-managed daily/monthly indexes are read-only adapters**: Use `ElasticReadOnlyRepositoryBase<T>`. Write methods can call `EnsureIndexAsync()` and create library-owned versioned partitions. Queries without `.Index(...)` and large-range fallbacks require the external system to maintain the `{Name}` umbrella alias. Override `GetIndexMappingFilter()` and `GetIndexDate()` together; the filter is authoritative and malformed wildcard matches returning `DateTime.MaxValue` are ignored.
