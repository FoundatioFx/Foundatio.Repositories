...
- **Elasticsearch major-version compatibility reindexing is explicit and rollback-sensitive**: Normal `ConfigureIndexesAsync` has zero compatibility-check overhead. Use `GetIndexCompatibilityAsync()` as a preflight, take a verified snapshot, and call `IElasticConfigurationCompatibility.UpgradeIndexCompatibilityAsync()` only after the server rollback window closes. The operation creates `{name}-r{n}` physical indexes under the current server major and deletes the old physical indexes; application aliases remain stable, but an older server cannot open the new indexes. See [index-lifecycle.md](references/index-lifecycle.md#explicit-index-compatibility-upgrades-es-major-version-upgrades).
