...
- **Elasticsearch major-version compatibility reindexing is explicit and offline**: it never runs automatically. Preflight with `GetIndexCompatibilityAsync()`, then call `UpgradeIndexCompatibilityAsync()` one major at a time, after the rollback window closes. Generated destinations must not collide with any other registration's logical/native/error names or time-series aliases, including uncreated indexes outside the requested batch. Ordinary progress-callback exceptions are logged; cancellation still propagates through evidence-based handling. See [index-lifecycle.md](references/index-lifecycle.md#explicit-index-compatibility-upgrades-es-major-version-upgrades).
