...
- **`ExistsAsync(id)` with soft deletes is also a dirty read**: When the model implements `ISupportSoftDeletes`, `ExistsAsync(id)` falls back to the Search API to apply the `IsDeleted` filter. Only models without soft deletes get the realtime Document Exists API path.
