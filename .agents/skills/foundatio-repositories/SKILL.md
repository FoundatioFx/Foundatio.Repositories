...
- **Document counts are compared but never authoritative — except under quiesce**: `VerifyDocumentCountsAsync` runs on every reindex (it used to run only when `DiscardIndexesOnReindex` was set) and with the default ordering only ever warns — a short destination keeps the old index rather than throwing. It cannot be authoritative there because the alias is already switched when it runs: hard deletes through the alias shrink the destination while the frozen source still counts them, and post-cutover writes can mask a genuinely short copy. Completeness comes from the per-pass task accounting instead. Under `QuiesceSourceOnReindex` the same comparison **is** a hard gate that throws `ReindexIncompleteException`, because the source is blocked and the alias has not moved, so a shortfall has no benign explanation and refusing costs nothing.
...
- **A copy that cannot catch up is refused if its source changed**: with no `TimestampField` and non-ObjectId ids there is no way to find documents written during the copy, so the reindex compares the source's highest `_seq_no` before the copy and before the alias switch. If it advanced, it throws `ReindexIncompleteException` and refuses to promote (alias stays on the old index, which is retained). A static source is unaffected, so date-free models and custom ids remain supported — the trigger is the missed catch-up, not the id format. Enforced pre-switch on purpose; a post-switch throw would report the loss without preventing it. See [index-lifecycle.md](references/index-lifecycle.md#second-pass-catch-up).
- **ObjectId catch-up cannot see in-place updates, and now refuses them**: the ObjectId fallback ranges on `_id`, which encodes only *creation* time, so an update to a pre-existing document was silently dropped and the destination promoted anyway. A `_seq_no` baseline is now recorded in that branch too; if any document changed during the copy has an id sorting below the catch-up watermark `ObjectId(startTime)`, it throws `ReindexIncompleteException` before the cutover. Documents merely *created* during the copy sort above the watermark and are caught up normally, so append-heavy models don't false-positive — refusing on any `_seq_no` advance would fail every one of them. The check cannot see a delete (no sequence number remains). **This can fail migrations that previously appeared to succeed, but they were losing data.** Remedy is `QuiesceSourceOnReindex`.
- **The default ordering is not lossless — three defects, two hitting `IHaveDates`**: the alias switches *before* the catch-up pass, so (A) in-place updates missed by the ObjectId range — now refused; (B) **stale overwrite**: catch-up copies with `Conflicts.Proceed` + internal versioning + no `op_type`, so it unconditionally clobbers a post-cutover write with the older source copy — invisible to `version_conflicts` and to count comparison; (C) **resurrected deletes**: no slicing mode expresses a delete, so a post-cutover delete is re-copied — and the resurrection makes counts *match*. B and C affect **all models including `IHaveDates`** and are closed only by quiescing.
- **`QuiesceSourceOnReindex` closes A/B/C by moving the cutover last — at the cost of a write outage**: set on `Index<T>` (flows into `ReindexWorkItem.QuiesceSource`), it reorders to copy → block source writes → catch up to convergence → reconcile deletes → verify → switch → unblock. Under it the catch-up rescans the **whole** source (which is how in-place updates get caught, and makes `TimestampField`/ObjectId classification irrelevant), destination documents absent from the source are deleted rather than resurrected (skipped when a reindex `Script` is in use, since `ctx.op = 'noop'` is ambiguous), and count verification becomes a **hard gate** that throws instead of warning. **Writes to the source are rejected with `cluster_block_exception` for the whole copy, so the outage is proportional to index size** — that is why it is opt-in and off by default. Time-series blocks **one partition at a time**. Convergence is detected from the source's `_seq_no`, not copy counters: `_reindex` reports byte-identical rewrites as `updated`, so a counter-based loop never terminates. See [index-lifecycle.md](references/index-lifecycle.md#quiesced-cutover-quiescesource).
- **Completion is recorded durably, never inferred**: a finished migration writes a `ReindexCompletion` record to the `foundatio-reindex-completions` index before the queue item is acknowledged and before the source is deleted. A redelivered queued work item is acknowledged only when a matching record exists; if the alias is already promoted with no record it throws `ReindexCompletionUnknownException` — neither acknowledging nor recopying into the live destination, leaving both indexes untouched. An advanced alias is *not* evidence of completion, because with the default ordering the alias moves before the catch-up pass, so a post-cutover failure leaves exactly the state a success leaves. **A quiesced migration is the exception**: promotion there happens only after reconciliation and verification succeeded, so a missing record is re-derived (`PromotedAfterVerification`) and the item acknowledged. The record is keyed by the logical migration (alias + source + destination), not an attempt or delivery id, and is bound to the destination's UUID and a fingerprint of the reindex script so a stale generation or a different transformation cannot vouch for it. Migrations completed before this release have no record. Also: `IElasticConfiguration.ReindexAsync` no longer retries a `ReindexIncompleteException` — the retry used to find the alias already moved, skip, and report success. See [index-lifecycle.md](references/index-lifecycle.md#completion-records).
