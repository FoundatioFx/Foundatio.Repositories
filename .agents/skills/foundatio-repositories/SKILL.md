...
- **Search-after sessions have strict invariants**: Direction is exclusive, cursor value count must match the final sort count, and snapshot paging cannot be mixed with PIT search-after. Raw null/empty/all-null arrays clear the cursor; decoded tokens preserve null sort values. Changing modes clears cursor/PIT/warning state, while reapplying the same mode preserves it. Scalar `FindOneAsync`, `CountAsync`, and query-based `ExistsAsync` reject PIT search-after options. Close caller-owned PITs when abandoning traversal; repository-owned PITs are closed on completion or failure.
