...
- **Elasticsearch major-version compatibility reindexing is explicit, offline, and rollback-sensitive**: Normal `ConfigureIndexesAsync` has zero compatibility overhead. After the rollback window closes, stop all writers and index-management jobs, take a verified snapshot, preflight with `GetIndexCompatibilityAsync()`, and call `UpgradeIndexCompatibilityAsync()`. It uses `_create_from`, blocks and drains source writes, disables ingest pipelines during the copy, verifies exact counts, and atomically moves aliases to `reindexed-v{major}-...`. Restart or drain clients before writes resume because cached concurrency tokens refer to the deleted physical index. See [index-lifecycle.md](references/index-lifecycle.md#explicit-index-compatibility-upgrades-es-major-version-upgrades).
