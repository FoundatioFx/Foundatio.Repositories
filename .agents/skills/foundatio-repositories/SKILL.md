...
- **Elasticsearch major-version compatibility reindexing is explicit, sequential, offline, and rollback-sensitive**: Normal `ConfigureIndexesAsync` has zero compatibility overhead. Run Elastic Upgrade Assistant, keep a verified snapshot, deploy client/server changes separately when practical, and upgrade one major at a time. After the rollback window closes, stop writers and index managers, preflight with `GetIndexCompatibilityAsync()`, reject `Unsupported`, and explicitly call `UpgradeIndexCompatibilityAsync()`. It validates a whole requested batch before its first mutation, rejects duplicate target lineages, revalidates source/schema ownership immediately before blocking writes, rejects destination-template drift, blocks the destination through cutover, preserves stable plus one canonical physical alias, and verifies tasks/shards/counts/full alias metadata. Ownership is exclusive across configured indexes (native `events-error` and `reindexed-v8-events` configurations outrank generated-name claims); generated `-error` partitions require the exact hidden `.foundatio-reindex-error` marker, while unmarked suffix collisions fail closed. A failed `_create_from`, malformed task listing, ambiguous target deletion, or uncertain cutover retains the source write block. Use `InspectIndexCompatibilityUpgradeAsync()` before the narrow recovery API; automatic target deletion requires the workflow's hidden ownership marker and confirmed task termination, while a completed topology requires that marker to be absent. Restart/drain clients before writes resume because cached concurrency tokens refer to the deleted physical index. See [index-lifecycle.md](references/index-lifecycle.md#explicit-index-compatibility-upgrades-es-major-version-upgrades).
