...
- **Paging resets include the page number**: Disabling search-after or changing Live/PIT mode resets to page one and preserves the page limit. Backward paging must preserve multivalue selection and reverse missing placement as well as direction. Failed PIT searches (including HTTP 404) throw `DocumentException`. After successful failure cleanup, `NextPageAsync()` rejects the cleared session; restart with `FindAsync`. Results remain bound to their original Live or PIT session across hooks and cannot attach to a replacement session. Reapplying the same mode preserves identity; disabling and re-enabling it does not. Do not share mutable options/results between concurrent traversals. Live/PIT cursor requests disable partial results and reject HTTP 200 timeouts or shard failures before advancing. Retained sessions use the latest options PIT ID. Scalar Live cursors bypass caches after `BeforeQuery`, reject incomplete search responses, and reject snapshot/async paging combinations. Backward query builds copy every sort variant and preserve settings, including date formats. PIT cleanup tests must assert the latest PIT ID is closed or inspect open search contexts, not scroll counts.
