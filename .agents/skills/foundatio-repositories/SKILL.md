...
- **`PatchAllAsync` sends notifications per-batch**: Notifications are sent incrementally as each batch is processed, not accumulated and sent after the entire operation completes. Subscribers may see `EntityChanged` messages while the operation is still running. Design subscribers to be idempotent.
