...
- **Elasticsearch major-version compatibility reindexing is explicit, offline, and rollback-sensitive**: `ConfigureIndexesAsync` adds zero compatibility requests. Upgrade one Elasticsearch major at a time; after the rollback window closes, stop writers and index managers, verify a snapshot, preflight with `GetIndexCompatibilityAsync()`, then explicitly call `UpgradeIndexCompatibilityAsync()`. Public recovery only finishes verified committed cutovers. Empty task listings do not authorize reset after ambiguous requests or restarts. Preflight rejects source-pruned mappings and generated names over 255 UTF-8 bytes. See [index-lifecycle.md](references/index-lifecycle.md#explicit-index-compatibility-upgrades-es-major-version-upgrades).
- **Compatibility upgrade exceptions do not imply rollback**: batches commit per physical index, and final verification can observe cancellation after cutover. See the lifecycle reference for callback and recovery behavior.
