...
- **Failure semantics**: cancellation and pre-cutover failures cancel the server task, delete only the known destination, and remove only a write block added by this run. Cleanup failure is surfaced explicitly. An ambiguous cutover fails closed with instructions to inspect both physical indexes and aliases; it is never automatically cleaned up.
- **Unsupported sources**: reject Elasticsearch before 8.18, system/dot indexes, data-stream backing indexes, `_source` disabled, non-standard index modes, ILM-managed indexes, and CCR followers.
- **Testing**: detection tests cover version parsing, cancellation, lock recomputation, request overhead, and prefix/date parsing. live upgrade tests cover plain/versioned/daily cutover, complete alias metadata, ingest-pipeline suppression/restoration, active write fencing, cancellation cleanup, destination collisions, schema precedence, post-compatibility schema reindex, lifecycle deletion, and the Elasticsearch 8.18 minimum.
