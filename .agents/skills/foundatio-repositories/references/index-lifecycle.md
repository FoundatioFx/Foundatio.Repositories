...
**A copy that cannot be confirmed fails rather than reporting quiet success** (see "Reindex failure is never silent" below). Do not infer rollback or retry permission from the exception: alias requests and callbacks can fail after promotion. Inspect exact aliases and durable task/block/completion state; only a permitted replay copies from the beginning. Recovery may need a fresh destination built from authoritative application data.
...
**Concurrency:** *Within an index* a reindex is strictly sequential — one partition at a time (`await`ed `foreach` in `DailyIndex.ReindexAsync`), and each ES `_reindex` is a single **unsliced** task (library never sets `slices`). *Across indexes* it depends on the trigger: `configuration.ReindexAsync()` runs them sequentially (`foreach`), but `ElasticMigrationJob` runs them **in parallel** (`Task.WhenAll`, one task per outdated index). A shared distributed lock keyed on the alias (`reindex:<alias>`, held 20 min and renewed on progress) is intended to serialize a logical migration. This requires continuously valid ownership; it does not prove single server-task dispatch, prevent transport retries, or fence a stale controller after lease loss. So a single-index upgrade needs ~one partition of disk headroom; parallel multi-index runs need ~one in-flight partition per concurrent index.
...
**Alias cutover is not a copy transaction:** writes use the unversioned dated alias (`audit-2024.01`). The alias definitions are moved together, but default-mode catch-up still follows promotion and can race with newer writes/deletes. Quiesced mode reconciles before promotion while the physical source is blocked. Do not claim losslessness merely from an atomic alias update.

**Why oldest → newest:** older partitions commonly receive less traffic, but backfills, corrections, deletes, and retention can still change them. Each quiesced partition uses a full final rescan regardless of age. Migration progresses partition by partition; an interrupted post-promotion attempt requires explicit recovery, not only the configured-version dispatcher. There is no global atomic migration of all partitions.
...
// Cancellation is not rollback: inspect alias/task state after a dispatched cutover request.
await configuration.ReindexAsync(cancellationToken: stoppingToken);
```

The original `ReindexAsync` overloads remain callable, with additive cancellation-aware overloads. Before cutover the caller token is observed and cleanup uses independent bounded tokens. After default-mode promotion starts, required catch-up does not inherit caller cancellation; a lost/cancelled alias response can already have applied. The queued path forwards the work-item cancellation token. `ReindexTaskLease` attempts to stop the recorded server task during unwinding, but a recorded task ID is not complete authority over unknown duplicate submissions or task lineage. Neither an exception nor task 404 proves rollback or permission to replay. See the [outcome table](../../../../docs/guide/reindex-safety.md#cancellation-and-outcome-handling) and [current release gates](../../../../docs/guide/reindex-safety.md#current-release-gates).
...
**Document counts are compared on every reindex** (`VerifyDocumentCountsAsync`), not only when `DiscardIndexesOnReindex` is set — the check used to live inside the delete branch, so a `DeleteOld = false` reindex was never checked at all. It **never throws**: it warns and returns `false`, which keeps the old index. It deliberately isn't authoritative, because the aliases are already switched by the time it runs — documents hard-deleted through the alias shrink the destination while the frozen source still counts them (throwing there would force a retry that resurrects the deleted documents), scripts can drop documents via `ctx.op`, and post-cutover writes can offset genuinely-failed copies and mask a real shortfall. Per-pass accounting proves task completion, not concurrent consistency. Quiesced mode uses a separate exact pre-promotion gate.

**Required source/destination refreshes fail closed.** Missing or failed shard evidence cannot establish that a copy/count sees acknowledged writes. Only an absent, not-yet-created destination is allowed before its initial copy.

The same refresh runs **before catch-up planning samples the source**, for a sharper reason: planning reads one document to decide whether ids are ObjectIds, and zero hits is interpreted as "the source is empty", which previously disabled both catch-up and the change guard; the current empty-source plan retains per-primary checkpoint checks. A source bulk-loaded with `refresh_interval: -1` would look empty to planning while the copy — which refreshes first — copied a full index, promoting it with no catch-up and no refusal gate. Guarded by `ReindexTests.ReindexAsync_WhenSourceHasUnrefreshedWrites_DoesNotTreatItAsEmpty`.
...
A client crash does not stop an asynchronous `_reindex` task or remove an index write block. The `foundatio-reindex-safety` journal records task dispatch intent and known task IDs, plus source-UUID-bound write-block intent/confirmation. The implementation checks its recorded task before another dispatch. Unknown task IDs remain fenced; cancellation acknowledgement alone is insufficient. Task 404 and one recorded ID do not prove that every retried/relocated task or delayed controller is stopped; the task-lineage and effective single-dispatch gates remain open. Proven same-generation migration blocks are recoverable; operator blocks and ambiguous ownership are not cleared automatically. Owner-token/OCC checks prevent stale journal mutations.

A safe retry copies from the source again, without a destination timestamp watermark. An already-promoted destination without matching completion is not replayable in either mode. Configuration version discovery is not completion verification. See `docs/guide/reindex-safety.md` for operating assumptions and recovery steps.

### Second-Pass Catch-Up

Default mode selects timestamp catch-up when available, otherwise ObjectId `id` ranges. For unsupported IDs, it compares generation-bound checkpoint vectors for every primary shard. Initially empty sources also receive the change guard. For ObjectId catch-up, it scrolls **all** changed documents per primary shard using that shard's baseline, verifying actual `_id` and indexed `id` against the watermark; it never samples only the first 1,000 changes.

These non-blocking checks are refusals, not write barriers. Hard deletes leave no searchable source document; post-cutover writes/deletes can still race with catch-up. Timestamp models are not exempt. Use quiesced mode for verified mutable-data cutover.

### Quiesced cutover (`QuiesceSource`)

The first full pass is live. After it completes, `IndexWriteBlock` uses the dedicated add-block API to block and drain source writes. It validates acknowledgement and per-index confirmation. Final copying/reconciliation/verification precede atomic alias promotion; owned blocks are released before completion recording and optional source cleanup. Time-series holds one partition block at a time.

The final copy scans the entire source independent of timestamps/ID formats. It verifies a generation-bound vector for every primary before/after reconciliation. Copy counters cannot establish source stability: byte-identical writes can still increment `updated`.

Unscripted deletion reconciliation scans all destination `(id,routing)` keys, even at equal counts, uses routed `_mget`, and validates every lookup and bulk-delete result. A read error is never an absence. Scripted mode clears the unpromoted destination before its full final copy so newly no-op/deleted output cannot survive from pass one. Expected final cardinality is `total - noops - deleted`; undercounts and overcounts fail before promotion. `StartUtc` is rejected for quiesced promotion.

Both task and block cleanup are independently bounded and durable ownership is required for recovery. Unknown outcomes need operator inspection, not fabricated completion or blind replay. The write outage scales with full-source and destination work, not just changed records; no large-production-index duration is claimed. See the public reindex-safety guide and `ReindexSafetyTests`/`ReindexRecoveryTests`.

### Completion records

`foundatio-reindex-completions` stores one record per logical physical migration after required work succeeds and before source deletion. Its versioned, bounded SHA-256 ID encodes the alias/source/destination tuple unambiguously. Validation checks identity fields, destination UUID, and a fingerprint of script, timestamp field, start range, and quiesce mode. Throughput and cleanup settings are intentionally excluded.

A matching record is acknowledged without replay. An unpromoted destination may be copied only when the source still owns its alias (or that alias is absent) and task/block safety state permits it. A promoted destination with no matching record is **unconfirmed in either mode**. The removed `PromotedAfterVerification` branch incorrectly inferred a prior attempt's protocol from a new request flag; it must not be restored.

Unavailable completion or generation reads fail closed. Old record formats are not accepted as the new contract. Completion does not turn the default non-blocking mode into lossless online migration. Ordinary startup only discovers versions; explicit recovery uses exact work-item identities.
...
| Error | `Failed to get the status {N} times in a row for reindex task ... reindexing {OldIndex} -> {NewIndex}` | Status polling exceeded its failure budget. Cleanup attempts to stop the task and confirm termination; unconfirmed dispatch/termination leaves a durable replay fence for inspection |
...
Use [Reindex Safety and Recovery](../../../../docs/guide/reindex-safety.md) for destination exclusivity, producer retry/idempotency, pipeline and routing constraints, cancellation outcomes, least-privilege checks, retention coordination, and acceptance. “Verified” is protocol accounting and membership/cardinality validation, not a byte-for-byte checksum.
