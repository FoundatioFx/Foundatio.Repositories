...
- **Zero default overhead**: normal configuration performs no server-info or compatibility-settings requests. `GetIndexCompatibilityAsync(cancellationToken)` is an operator-invoked preflight using one fresh `Client.InfoAsync()` plus one settings-only `Indices.GetAsync` through the stable alias. The request expands hidden indexes and includes every still-existing daily partition, even if retention considers it expired; only a confirmed deletion removes it from the result. The server version is not cached because the process may survive a rolling cluster upgrade. An explicit upgrade checks before `reindex:{name}`, recomputes after acquiring the lock, and verifies once more after cutover.
