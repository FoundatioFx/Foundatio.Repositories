...
- **Zero default overhead**: normal configuration performs no server-info or compatibility-settings requests. `GetIndexCompatibilityAsync()` is an operator-invoked preflight using one fresh `Client.InfoAsync()` plus one `Indices.GetAsync` through the stable index alias, shaped with `Feature.Settings` and `IncludeDefaults(false)`. The server major is not cached across checks because the same process may survive a rolling cluster upgrade. A daily/monthly alias still resolves all retained partitions in one settings request, and mappings and orphaned revision names are not materialized. An explicit upgrade checks before the lock, recomputes inside the lock to prevent stale work-item races, and verifies once more afterward so incomplete reindexes fail loudly.
- **Rollback boundary**: Elasticsearch does not support downgrading upgraded nodes. A rollback requires rebuilding the older cluster and restoring a pre-upgrade snapshot. `UpgradeIndexCompatibilityAsync()` makes that boundary even more concrete: it creates physical indexes under the current server major and deletes the older physical indexes. Do not call it during the server rollback window.
- **Minimal API**: compatibility lives on optional `IIndexCompatibility : IIndex` and `IElasticConfigurationCompatibility` capability interfaces. Existing `IIndex` and `IElasticConfiguration` implementations gain no required members, and `ReindexWorkItem` construction stays internal to `ElasticConfiguration`.
- **Physical names change; application names do not**: a plain `employees` index becomes physical `employees-r1` with `employees` as an alias; `employees-v2` becomes physical `employees-v2-r1` while `employees` and `employees-v2` remain aliases; `logs-v1-2024.01.15` becomes physical `logs-v1-2024.01.15-r1` while its dated, umbrella, and windowed aliases move. Time-series upgrades do not retain the old physical name as another alias, avoiding one permanent alias per partition. A later compatibility reindex increments the suffix (`-r2`, etc.).
- **Remediation**: destinations are created through the concrete `Index` configuration, preserving settings and mappings. Work items always use the logical index name for `reindex:{name}` locking.
- **Schema-version precedence**: a pending schema reindex already creates current-server indexes, so compatibility work does not duplicate it. Daily/monthly detection filters the stable alias response to partitions on the configured `Version`; older-schema partitions are left for the schema upgrade.
- **Alias, source, and destination safety**: `ElasticReindexer` moves source aliases atomically after the first pass and preserves their filters, routing, hidden status, and write-index status; its second pass catches writes made to the source during the first pass. First-time plain/single-version upgrades use `remove_index` plus `add` after both passes and count verification to replace the old concrete name with an alias; later revisions move existing aliases normally. A stale work item whose old alias already resolves to its destination is a no-op. Under the lock, all proposed destination names are checked in one names-only request; an existing destination fails the operation before reindexing so unrelated or partial data is never silently merged. After reindexing, a second batched names-only check verifies that all old physical sources were removed, including time-series sources whose stable aliases already point at the new indexes.
- **Concurrency**: `UpgradeIndexCompatibilityAsync()` does a pre-check, acquires `reindex:{name}`, then recomputes work items inside the lock.
- **Testing**: `IndexCompatibilityTests` covers zero configuration probes, revision/version parsing, current server-version reads, unavailable-version failure, cancellation, and lock rechecks. `IndexCompatibilityUpgradeTests` covers the explicit request budget, stable-alias discovery, orphan exclusion, mapped destination creation, plain/versioned/time-series alias behavior, stale work-item safety, and reindex behavior. `ElasticIndexExtensionsTests` pins the settings-only request shape.
