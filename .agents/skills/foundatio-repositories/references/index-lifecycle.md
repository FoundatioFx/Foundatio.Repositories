...
A client crash does not stop an asynchronous `_reindex` task or remove an index write block. The `foundatio-reindex-safety` journal records task dispatch intent and known task IDs, plus source-UUID-bound write-block intent/confirmation. Known tasks must be confirmed stopped before another dispatch. Unknown task IDs remain fenced; cancellation acknowledgement alone is insufficient. Proven same-generation migration blocks are recoverable; operator blocks and ambiguous ownership are not cleared automatically. Owner-token/OCC checks prevent stale journal mutations.
