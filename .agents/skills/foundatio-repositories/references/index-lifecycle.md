...
- **Failure and recovery semantics**: task cancellation uses `wait_for_completion`; target deletion/unblocking is forbidden when task termination is unknown. `InspectIndexCompatibilityUpgradeAsync` is read-only and reports topology, aliases, blocks, and matching tasks. `RecoverIndexCompatibilityUpgradeAsync` holds `reindex:{name}` and only deletes an unaliased pre-cutover destination after zero matching tasks; write-block removal is a separate explicit opt-in. Ambiguous cutovers never auto-heal or choose a “newer” index.
