...
- **Zero default overhead**: normal configuration performs no server-info or compatibility-settings requests. `GetIndexCompatibilityAsync(cancellationToken)` is an operator-invoked preflight using one fresh `Client.InfoAsync()` plus one settings-only `Indices.GetAsync`; plain/versioned indexes use their stable alias, while daily indexes use one wildcard request over all still-existing partitions. The request expands hidden indexes and includes partitions that retention considers expired; only a confirmed deletion removes one from the result. The server version is not cached because the process may survive a rolling cluster upgrade. An explicit upgrade checks before `reindex:{name}`, recomputes after acquiring the lock, and verifies once more after cutover.
...
- **Dedicated implementation**: compatibility uses `ElasticIndexCompatibilityUpgrader` and `ElasticReindexTaskRunner`; it does not add compatibility branches or flags to the shared `ElasticReindexer`/`ReindexWorkItem` schema-migration path.
- **Kibana lifecycle plus Foundatio hardening**: validate source and destination, add a write block with the dedicated block API (which drains in-flight writes), refresh, `_create_from`, async `_reindex`, restore settings, then atomically add aliases and `remove_index`. The destination temporarily uses zero replicas, refresh `-1`, and default/final pipeline `_none`; reindex also uses destination pipeline `_none`, `op_type=create`, conflict abort, and automatic slicing. Exact task results and source/target counts are required before cutover.
- **Physical names change; application names do not**: `employees` becomes `reindexed-v9-employees`; `employees-v2` becomes `reindexed-v9-employees-v2`; `logs-v1-2024.01.15` becomes `reindexed-v9-logs-v1-2024.01.15`. A later major replaces the prefix rather than chaining it. Versioned/daily lifecycle parsing, mapping lookup, schema reindex, retention, and deletion canonicalize this prefix. A configured name that naturally starts with `reindexed-v8-` is not stripped; it becomes `reindexed-v9-reindexed-v8-...`. Old physical names are not retained as aliases.
- **Schema-version precedence**: compatibility candidates whose canonical schema version differs from the configured `Version` fail before any write block and instruct the operator to run the schema reindex first. This prevents a compatibility copy from duplicating or bypassing a pending application schema migration.
- **Alias and index-time fidelity**: complete alias definitions are copied (filter, routing, hidden, and write-index metadata). `_create_from` preserves source mappings/settings. Default and final ingest pipelines are disabled only for the existing-document copy and then restored, preventing non-idempotent processors from running twice.
- **Failure semantics**: cancellation and pre-cutover failures cancel the server task, delete only the known destination, and remove only a write block added by this run. Cleanup failure is surfaced explicitly. An ambiguous cutover fails closed with instructions to inspect both physical indexes and aliases; it is never automatically cleaned up. A lost response while adding the source write block is also treated as uncertain and requires manual inspection before retrying.
- **Unsupported sources**: reject Elasticsearch before 8.18, system/dot indexes, data-stream backing indexes, `_source` disabled, non-standard index modes, ILM-managed indexes, CCR followers, and read/metadata/read-only blocks. A pre-existing write-only block is preserved on the replacement index.
- **Testing**: detection tests cover version parsing, cancellation, lock recomputation, request overhead, and prefix/date parsing. live upgrade tests cover plain/versioned/daily cutover, complete alias metadata, ingest-pipeline suppression/restoration, active write fencing, cancellation cleanup, existing write-block preservation, destination collisions, schema precedence, post-compatibility schema reindex, lifecycle deletion, and the Elasticsearch 8.18 minimum. Background schema migration resolves the current prefixed physical source before enqueueing work.
