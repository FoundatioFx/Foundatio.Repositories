...
- **Zero default overhead**: normal configuration performs no server-info or compatibility-settings requests. `GetIndexCompatibilityAsync(cancellationToken)` is an operator-invoked preflight using one fresh `Client.InfoAsync()` plus one settings-only `Indices.GetAsync`; versioned and time-series patterns cover every structurally valid physical index in one request, including retained/expired indexes and hidden partitions. The server version is not cached because the process may survive a rolling cluster upgrade. An explicit upgrade checks before `reindex:{name}`, recomputes after acquiring the lock, and verifies once more after cutover.
...
- **Dedicated implementation**: compatibility uses `ElasticIndexCompatibilityUpgrader` and `ElasticReindexTaskRunner`; no compatibility flags leak into `ReindexWorkItem`. The shared schema reindexer rejects filtered/routed source aliases, atomically moves ordinary aliases after the first pass, and catches up from the resolved concrete source so the moved alias cannot redirect the second pass.
- **Kibana lifecycle plus Foundatio hardening**: validate source and destination, always call the dedicated write-block API, require cluster/shard/exact-index block acknowledgement, reject partial refresh/count shards, `_create_from`, async `_reindex`, restore settings, block the target through cutover, and wait for primary availability. Re-read alias definitions before atomic `remove_index`, reconcile with an independent token afterward, and compare full alias metadata. Exact task results and source/task/target counts are required.
...
- **Schema-version precedence**: a compatibility candidate whose schema version differs from the configured `Version` and still owns the logical alias fails before any write block and instructs the operator to run the schema reindex first. An unaliased old version intentionally retained with `DiscardIndexesOnReindex = false` is inactive and can be compatibility-upgraded independently.
...
- **Failure and recovery semantics**: task cancellation uses `wait_for_completion`; target deletion/unblocking is forbidden when task termination is unknown. A transport failure or accepted start response without a task ID is also uncertain: retain the target and source write block until matching Elasticsearch tasks are inspected. `InspectIndexCompatibilityUpgradeAsync` is read-only and reports topology, aliases, blocks, and matching tasks. Inspect/recover reject expressions and concrete source names not structurally owned by the supplied index before any request or lock. `RecoverIndexCompatibilityUpgradeAsync` holds `reindex:{name}` and only deletes an unaliased pre-cutover destination after zero matching tasks; partial task listings with `node_failures` or `task_failures` are unknown and fail closed. Write-block removal is a separate explicit opt-in. Ambiguous cutovers never auto-heal or choose a “newer” index.
...
- **Testing**: unit/request-budget tests cover derived compatibility state, version parsing, cancellation, lock recomputation, zero startup compatibility I/O, one preflight settings request across all partitions, and natural-prefix parsing. Live tests cover plain/versioned/daily cutover, canonical aliases, full alias metadata, pipelines, write fences, confirmed task cancellation, recovery classification, destination collisions, schema precedence, and the Elasticsearch 8.18 minimum. CI also carries one persistent data directory through ES7→ES8→ES9 without changing `VersionedIndex.Version`.
