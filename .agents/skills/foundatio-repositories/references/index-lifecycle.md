...
**`IndexWriteBlock`** (`Repositories/IndexWriteBlock.cs`) is the primitive: an `IAsyncDisposable` applying `PUT /{index}/_block/write` through the **raw transport**, because `Elastic.Clients.Elasticsearch` 8.19.23 exposes no typed write-block API. It validates `shards_acknowledged` *and* the per-index `blocked` flag rather than trusting a 200. There is no unblock API, so release is `PUT _settings` with `{"index.blocks.write": null}` — issued via raw transport because the typed settings descriptor serializes a `null` block to an empty object. Release deliberately passes **no** cancellation token: threading the caller's cancelled token through makes `DisposeAsync` throw and leaves the index read-only, masking the cancellation (pinned by `DisposeAsync_WhenCallerIsCancelled_StillRestoresWrites`). The settings response types `blocks.write` as the **string** `"true"`, not a boolean, so parsing accepts both. A pre-existing block is detected and **left in place** on dispose (something else owns it). Release failure logs at Error and surfaces if no other exception is in flight — a stuck block leaves an index read-only.
