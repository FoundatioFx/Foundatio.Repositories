...
- **Recovery actions**: inspection returns `None`, `Wait`, `Reset`, `Finish`, or `ManualIntervention`. Reset deletes only a marked partial target after the exact task is absent, then unblocks and unmarks the marked source. Finish unblocks and unmarks only a marked committed target with the canonical alias. Error lineage needs `.foundatio-reindex-error` on every surviving artifact. Unmarked, prior-major, multi-target, duplicate-task, partial-task-list, or contradictory states are never mutated.
- **Names and Kibana**: physical names become `reindexed-v{major}-{canonical}` while repository aliases remain stable; a later major replaces the prior generated prefix. Kibana uses the same namespace and `_create_from`, but has a richer Saved Object state machine. Never run both workflows on one index concurrently; Foundatio cannot resume Kibana and never deletes its unmarked destination.
