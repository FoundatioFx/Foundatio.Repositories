...
Forward and backward cursors are mutually exclusive and the cursor value count must equal the final sort count. Raw null/empty/all-null arrays clear the cursor; token-decoded arrays preserve null sort values. Changing Live/PIT mode clears the paging session; reapplying the same mode preserves it, and the final mode is evaluated after `BeforeQuery` handlers. Snapshot and async queries are mutually exclusive with search-after, scalar query operations reject PIT search-after, and all search-after requests bypass query-result caching because cache keys do not include cursor tuples. `SearchAfterPaging(false)` clears the complete local session. Close an abandoned caller-owned PIT with `ISupportPointInTime.ClosePointInTimeAsync(results)`; repository-owned PITs close on completion or failure.
