...
The repository appends the id's sort-safe mapped field (for example `id.sort`) as a tiebreaker when the model implements `IIdentity` and the index guarantees that field is sortable. Otherwise, Live mode requires an explicit stable, unique sort and throws `QueryValidationException` when none exists. [Elasticsearch adds `_shard_doc` as an implicit PIT tiebreaker](https://www.elastic.co/docs/reference/elasticsearch/rest-apis/paginate-search-results#search-after); point-in-time mode makes it explicit after caller sorts so the complete tuple can be reversed for backward cursors.

Forward and backward cursors are mutually exclusive and the cursor value count must equal the final sort count. Raw null/empty/all-null arrays clear the cursor; token-decoded arrays preserve null sort values. Changing Live/PIT mode clears the paging session and resets the page number to one while preserving the limit; reapplying the same mode preserves it, and the final mode is evaluated after `BeforeQuery` handlers. Snapshot and async queries are mutually exclusive with search-after, scalar query operations reject PIT search-after, and all search-after requests bypass query-result caching because cache keys do not include cursor tuples. `SearchAfterPaging(false)` clears the complete local session, including the page number. Backward paging preserves multivalue selection modes and reverses missing-value placement, not just sort direction. Close an abandoned caller-owned PIT with `ISupportPointInTime.ClosePointInTimeAsync(results)`; repository-owned PITs close on completion or failure. Failed PIT searches, including HTTP 404, throw `DocumentException` instead of returning an empty terminal page. Cleanup preserves the original exception. After cleanup clears the session, `NextPageAsync()` throws `QueryValidationException`; restart with `FindAsync`. Continuations validate the originating Live or PIT session before cursor mutation and after `BeforeQuery`; resetting in `AfterQuery` invalidates the returned continuation too. Separate mutable options/results per traversal and await each next page; query, sorts, and targets must remain unchanged. The active request retains cleanup responsibility if hooks clear its owned session; explicit `PointInTimeId(...)` transfers ownership instead. Live and PIT cursor searches set `allow_partial_search_results=false` and reject timeout/shard-failure metadata before hits, events, or cursor advancement. If cleanup fails or the caller owns the PIT, continuations use the latest ID in the options, even after `AfterQuery` fails. `FindOneAsync` and `CountAsync` run `BeforeQuery` before cache decisions and bypass both cache reads and writes for Live cursors. Those operations and query-based `ExistsAsync` reject incomplete Live responses and incompatible snapshot/async modes. Backward builds copy all supported sort variants, preserving caller objects and settings including `FieldSort.Format`; the public `ReverseOrder` helper itself remains in-place.
