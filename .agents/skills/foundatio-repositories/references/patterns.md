...
Forward and backward cursors are mutually exclusive and the cursor value count must equal the final sort count. Raw null/empty/all-null arrays clear the cursor; token-decoded arrays preserve null sort values. Changing Live/PIT mode clears the paging session; reapplying the same mode preserves it. Snapshot paging and PIT search-after are mutually exclusive, and scalar query operations reject PIT search-after. `SearchAfterPaging(false)` clears the complete local session. Close an abandoned caller-owned PIT with `ISupportPointInTime.ClosePointInTimeAsync(results)`; repository-owned PITs close on completion or failure.
